...
const NotesModal: React.FC<{
  ...
  const handleSave = async () => {
    ...
    await onSave(notes);
    setIsSaving(false);
    // Parent component will handle closing the modal by updating `editingNotesFor`
  };
    ...
    <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-[100] p-4 backdrop-blur-sm"> 
      <div className="neumorphic-card bg-brand-surface p-6 rounded-lg w-full max-w-md">
        ...
        <textarea
          ...
          className="neumorphic-input w-full mb-4 text-sm"
        />
        <div className="flex justify-end gap-3">
          <StyledButton onClick={onClose} variant="secondary_small" disabled={isSaving}>
            Cancel
          </StyledButton>
...
export default function WatchlistPage({ onViewDetails, onBack }: WatchlistPageProps) {
  ...
  const handleSaveNotes = useCallback(async (newNotes: string) => {
    if (!editingNotesFor || !editingNotesFor.anime) { 
        toast.error("Watchlist item or anime data not found to save notes.");
        setEditingNotesFor(null); 
        return;
    }
    const { _id: watchlistEntryId, animeId, status, progress, userRating } = editingNotesFor;

    try {
      const toastId = `notes-${watchlistEntryId}`;
      toast.loading("Saving notes...", {id: toastId});
      ...
      const toastId = `notes-${watchlistEntryId}`;
      toast.error(error.data?.message || error.message || "Failed to save notes.", {id: toastId});
    }
  }, [upsertToWatchlist, editingNotesFor]);


  const filteredWatchlist = watchlistDataFull?.filter(item =>
    (filterStatus === "All" || item.status === filterStatus) && item.anime 
  );
        ...
        {(["All", "Watching", "Completed", "Plan to Watch", "Dropped"] as WatchlistStatusFilter[]).map(status => (
          ...
          >
            ...
            {watchlistDataFull && (
              <span className="ml-1">
                ({status === "All" 
                  ? watchlistDataFull.filter(item => item.anime).length 
                  : watchlistDataFull.filter(item => item.status === status && item.anime).length})
              </span>
            )}
          </StyledButton>
        ))}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4">
          {filteredWatchlist.map((item) => (
            <div key={item._id} className="flex flex-col"> 
              ...
              {item.notes && (
                   <p className="mt-1 text-xs text-brand-text-secondary italic truncate p-1 bg-brand-dark rounded" title={item.notes}>
                      Note: {item.notes}
                  </p>
              )}
            </div>
